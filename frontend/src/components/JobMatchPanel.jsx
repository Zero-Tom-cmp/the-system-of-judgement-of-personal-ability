        ...
        <div style={{ marginTop: 12 }}>
          {topJob.dim_matches.map((dm) => (
            <div key={dm.dimension} style={{ display: "flex", alignItems: "center", marginBottom: 6 }}>
              ...
              {dm.insufficient ? (
                <span style={{ flex: 1, margin: "0 8px", fontSize: 12, color: "#999" }}>数据不足</span>
              ) : (
                <Progress percent={dm.match_rate} size="small" strokeColor={getColor(dm.match_rate)} style={{ flex: 1, margin: "0 8px" }} />
              )}
              <span style={{ width: 100, fontSize: 12, color: "#888" }}>
                {dm.insufficient ? "—" : `${dm.student_score}/${dm.required}分`}
              </span>
            </div>
          ))}
        </div>
